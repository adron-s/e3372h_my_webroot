        ...
        if (ret) {
            ...
            if ($.isArray(ret)) {
                ...
                $(ret).each( function(i) {
                    ...
                    addFilter(
                    $('#service_list tr'),
                    spaceToNbsp(XSSResolveCannotParseChar(ret[i].SpecialApplicationTriggerName)),
                ...
                addFilter(
                $('#service_list tr'),
                spaceToNbsp(XSSResolveCannotParseChar(ret.SpecialApplicationTriggerName)),
...
function comparePort(port,showTarget) {
    var TriggerPortArray=[];
    $('.user_add_line').each( function(i) {
        TriggerPortArray.push($(this).children().eq(2).text());
    });
    for(i=0;i<TriggerPortArray.length;i++) {
        if(TriggerPortArray[i]==port) {
            showQtip(showTarget, firewall_hint_Trigger_port_same);
            return false;
        }
    }
    return true;
...
$(document).ready( function() {
    ...
    $('.button_edit_list').live('click', function() {
        if (($(".add_item_control:hidden").size() > 0) && ($('#edit_item_ok').size() < 1)) {
            ...
            var SATriggerNameStr = SATriggerName.html();
            SATriggerNameStr = nbspToSpace(SATriggerNameStr);
            ...
            SATriggerName.html('<input type="text" value="' + XSSResolveHtmlReturnChar(SATriggerNameStr) + '" id="sa_trigger_name" maxlength="30"/></td>');
            SATriggerPort.html('<input type="text" value="' + XSSResolveHtmlReturnChar(SATriggerPort.html()) + '" id="sa_trigger_port"></td>');
            SAOpenPort.html('<input type="text" value="' + XSSResolveHtmlReturnChar(SAOpenPort.html()) + '" id="sa_open_port"></td>');
            ...
            $('#sa_trigger_status').attr('style','width:70px');
    ...
    function checkInputValue() {
        ...
        if (!comparePort($.trim($('#sa_trigger_port').val()), 'sa_trigger_port')) {
            return false;
        }
    ...
    function addNewAppItem() {
        ...
        if (checkInputValue()) {
            ...
            spaceToNbsp(XSSResolveCannotParseChar(inputValue.SATriggerName)),
    ...
    $('#edit_item_ok').live('click', function() {
        if (checkInputValue()) {
            ...
            hideAddItemControl();
            var editSpecialApp = $(this).parent().siblings();
            editSpecialApp.eq(0).html(spaceToNbsp(inputValue.SATriggerName));
    ...
    function postData() {
        ...
        $('.user_add_line').each( function(i) {
            ...
            var LanPort = {
                SpecialApplicationTriggerName: nbspToSpace($(this).children().eq(0).html()),
