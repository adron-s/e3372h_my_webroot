...
function isVaildValue() {
    ...
    if ('0' != g_config_firewall.lanipfilter.wan) {
        ...
        if(isSameSubnetAddrs(lanIPFilterLanAddress,lanIPFilterWanAddress,subnetMask)) {
            ...
            if ($.isArray(filters)) {
                $(filters).each( function(i) {
                    ...
                    if ('0' == g_config_firewall.lanipfilter.wan) {
                        addLanipFilter(
                        $('#service_list tr'),
                        ipTranGet(XSSResolveCannotParseChar(filters[i].LanIPFilterLanStartAddress), filters[i].LanIPFilterSrcStartIPMask),
                        lanPort,
                        protocolStatus,
                        filterStatus
                        );
                        ...
                        addLanipFilter(
                        $('#service_list tr'),
                        ipTranGet(XSSResolveCannotParseChar(filters[i].LanIPFilterWanStartAddress), filters[i].LanIPFilterDestStartIPMask),
                        wanPort,
                        protocolStatus,
                        filterStatus
                        );
                ...
                if ('0' == g_config_firewall.lanipfilter.wan) {
                    addLanipFilter(
                    $('#service_list tr'),
                    ipTranGet(XSSResolveCannotParseChar(filters.LanIPFilterLanStartAddress), filters.LanIPFilterSrcStartIPMask),
                    lanPort,
                    protocolStatus,
                    filterStatus
                    );
                    ...
                    addLanipFilter(
                    $('#service_list tr'),
                    ipTranGet(XSSResolveCannotParseChar(filters.LanIPFilterWanStartAddress), filters.LanIPFilterDestStartIPMask),
                    wanPort,
                    protocolStatus,
                    filterStatus
                    );
...
$(document).ready( function() {
    ...
    $('.button_edit_list').live('click', function() {
        ...
        && ((firewall_status != null && firewall_status.FirewallMainSwitch == 1) && (firewall_status != null && firewall_status.FirewallIPFilterSwitch == 1))) {
            ...
            if ('0' != g_config_firewall.lanipfilter.lan_enable) {
                editLanIpAddr.html('<input type="text" value="' + XSSResolveHtmlReturnChar(editLanIpAddr.html()) + '" id="input_lan_ip_address"></td>');
                editLanPort.html('<input type="text" value="' + XSSResolveHtmlReturnChar(editLanPort.html()) + '" id="input_lan_ip_port"></td>');
            }
            if ('0' != g_config_firewall.lanipfilter.wan) {
                editWanIpAddr.html('<input type="text" value="' + XSSResolveHtmlReturnChar(editWanIpAddr.html()) + '" id="input_wan_ip_address"></td>');
                editWanPort.html('<input type="text" value="' + XSSResolveHtmlReturnChar(editWanPort.html()) + '" id="input_wan_ip_port"></td>');
            }
